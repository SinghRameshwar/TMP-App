 ...
 */
export const UnscheduledClockInOutApiCall = async (payload) => {
    try {
        const loginData = JSON.parse(await AsyncStorage.getItem('@loginData'));
        const response = await fetch(`${ApiConfig.url}/api/v1/sdl/schedules/${payload.unschuledId}`, {
            method: 'GET',
            cache: 'no-cache',
            headers: {
        ...
        try {
            ...
            //console.log('----------Final API---------',responseData);
            return responseData;
        } catch (jsonError) {
