 ...
 */
export const MEDMagSchedulesDetailsApi = async (payload) => {
    try {
        const loginData = JSON.parse(await AsyncStorage.getItem('@loginData'));
        const response = await fetch(`${ApiConfig.url}/api/v1/med/schedules/details?order_ids=${payload.order_ids}&client_id=${payload.client_id}&date=${payload.date}&time=${payload.time}`, {
            method: 'GET',
            cache: 'no-cache',
            headers: {
