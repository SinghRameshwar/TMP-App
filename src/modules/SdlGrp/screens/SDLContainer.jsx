...
import { SafeAreaView, View, Image, Text, StyleSheet, FlatList, TextInput, Alert, Platform } from 'react-native';
...
import { requestSDLTskListService } from '../../../redux/action';
import { useDispatch, useSelector } from 'react-redux';

const SDLContainer = ({ route, navigation }) => {
    ...
    const serviceResponse = useSelector((state) => state.SDLTASKListRedu.data);
    ...
    const apiCallonViewLoad = async () => {
        ...
        if (state.isConnected) {
            ...
            await dispatch(requestSDLTskListService({
    ...
    const onRowClickAction = (item) => {
        navigation.navigate('SDLClockinOutDeleteContainer', { data: item, tytleType: 'clock-in', date: moment(date).format('YYYY-MM-DD'),  callBackMethod: callBackMethod });
    }
            ...
            <TouchableOpacity onPress={() => onRowClickAction(item)}>
                <View style={styles.listRowHeader}>
                    <View style={styles.listRowHeaderText}>
                        <Text>
                            <Text style={styles.taskTitle}>{item.service_delivery_code_title}: </Text>
                            <Text style={styles.taskSubtitle}>{item.service_delivery_code}</Text>
                        </Text>
                    </View>
                    <Text style={styles.taskDate}>{item.date} | {item.service_time}</Text>
                </View>

                <View style={styles.listRowContent}>
                    <View style={styles.listRowDetails}>
                        <FontAwesome name="user-o" size={15} color="#14669A" style={[styles.icon, { marginRight: 7 }]} />
                        <Text style={styles.detailText}>{item.client_fname} {item.client_lname}</Text>
                        <Text style={styles.separator}> | </Text>
                        <Icon name="mail-outline" size={15} color="#14669A" style={styles.icon} />
                        <Text style={styles.detailText}>{item.client_id}</Text>
                    </View>
                    <View style={styles.listRowDetails}>
                        <Icon name="location-outline" size={15} color="#14669A" style={styles.icon} />
                        <Text style={styles.detailText}>{item.location.location_address}</Text>
                    </View>
                </View>
            </TouchableOpacity>
        </View>
    ...
    return (
        <SafeAreaView style={styles.container}>
            ...
            <View style={styles.overlay}>
                <NarationViewWithbutton title="SDL" navigation={navigation} calendarOpen={calendarOpen} date={date} />
